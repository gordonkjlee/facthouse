...
On a terminal, run init. Do not paste further commands into this fence. Enter accepts each default (pull off, keyword search, extra knobs at shipped defaults).

```bash
openmemory init
```

That writes `~/.openmemory/config.json` and prints an MCP snippet to paste. It asks data directory, optional transcript capture, semantic search, and More settings.

Or skip init and paste this. The server creates `~/.openmemory` on first boot; you are not asked those questions, and the defaults stay invisible until you write a config.
...
Every scripted init in this README passes `--yes`. The walk-through fence above is a single command; do not put other commands in that same block.
...
- **One directory, one memory** — Isolation is the directory, not a column.
...
The same walk-through as Quick Start (do not paste further commands into that fence). Scripted:
