...
describe("pullSources cursor", () => {
  it("ingests nested agent-transcripts JSONL and skips turn_ended", () => {
    ...
    const result = pullSources(db, [{ kind: "cursor", home }]);
  ...
  it("discovers a flat agent-transcripts JSONL as well as the nested layout", () => {
    const home = path.join(root, "cursor-home");
    const group = encodeCursorProjectDir("C:\\dev\\app");
    writeJsonl(
    ...
    expect(result.files).toBe(1);
    expect(result.events_inserted).toBe(3);
    expect(events(db)[0].client_session_id).toBe("sess-flat");
  });

  it("cwd filter uses Cursor encoding, not Claude Code encoding", () => {
    const home = path.join(root, "cursor-home");
  ...
  it("does not walk Composer SQLite, chats/, .txt, or files outside agent-transcripts", () => {
    const home = path.join(root, "cursor-home");
    const group = encodeCursorProjectDir("C:\\dev\\app");
    writeJsonl(
    ...
    const result = pullSources(db, [{ kind: "cursor", home }]);
    expect(result.files).toBe(1);
    expect(events(db).every((r) => r.client_session_id === "sess-real")).toBe(true);
  });
  ...
  it("a second pull of the same file is a no-op", () => {
    ...
    pullSources(db, [{ kind: "cursor", home }]);
    const second = pullSources(db, [{ kind: "cursor", home }]);
